...
Keep using Read for images and PDFs, and plain Bash for work that is not reading, searching or editing files: git, builds, tests, `ls`."#;
    ...
    #[test]
    fn a_v0_0_1_session_start_hook_is_upgraded_to_the_current_command_not_duplicated() {
        assert_session_start_upgrades_to_the_current_command(&v0_0_1_session_start_command());
    }

    #[test]
    fn a_session_start_hook_with_our_wrapper_but_not_our_heading_or_tail_is_kept_beside_ours() {
        ...
        for foreign in [different_heading, different_tail.as_str()] {
            ...
            assert_eq!(session.as_array().map(Vec::len), Some(2), "{foreign}");
        }
    }

    #[test]
    fn a_v0_0_1_subagent_start_hook_is_upgraded_to_the_current_command_not_duplicated() {
        let sandbox = Sandbox::new().with_lets_on_path(true);
        std::fs::create_dir_all(sandbox.claude("")).unwrap();
        let old = serde_json::json!({
            "hooks": [{"type": "command", "command": v0_0_1_subagent_start_command()}]
        });
        std::fs::write(
            sandbox.claude("settings.json"),
            serde_json::json!({"hooks": {"SubagentStart": [old]}}).to_string(),
        )
        .unwrap();

        let outcome = sandbox.install(Format::Text);

        assert!(body_text(&outcome).contains("updated the SubagentStart hook"));
        let subagent = &sandbox.settings_json()["hooks"]["SubagentStart"];
        assert_eq!(subagent.as_array().map(Vec::len), Some(1));
        assert_eq!(subagent[0]["hooks"][0]["command"], subagent_start_command());
    }
